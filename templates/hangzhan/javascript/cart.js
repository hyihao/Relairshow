...
//计算购物车金额
function TotalCartAmount(obj, webpath, goods_id) {
    if (isNaN($(obj).val())) {
        alert('商品数量只能输入数字!');
        $(obj).val("1");
    }
    $.ajax({
        type: "post",
        url: webpath + "tools/submit_ajax.ashx?action=cart_menpiao_update",
        data: {
            "goods_id": goods_id,
            "goods_quantity": $(obj).val()
        },
...
function updateForm(obj) {
    var goods_quantity = $(obj).val();
    var sell_price = $(obj).parent().parent().find("lable[name='sell_price']").html();
    var real_amount = goods_quantity * sell_price;
    $(obj).parent().parent().find(".real_amount").html(real_amount.toFixed(2));
    var total_amount = 0;
    $('.cart_table tr td .real_amount').each(function (index, dom) {
        total_amount += parseInt($(dom).html());
    });

    $('#total_amount').find('.total_amount').html(total_amount.toFixed(2));
}
//购物车数量加减
...
function CartAddMinus(obj, webpath, goods_id, num) {
    if (num > 0) {
        ...
        $(goods_quantity).val(parseInt($(goods_quantity).val()) + 1);
        //计算购物车金额
        TotalCartAmount($(goods_quantity), webpath, goods_id);
    } else {
        var goods_quantity = $(obj).next("input[name='goods_quantity']");
